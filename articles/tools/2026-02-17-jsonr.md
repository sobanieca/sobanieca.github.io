...
> Notice how script adds assertion agains response status. This is feature that one can use to define simple smoke tests. If response code is different from the
> one provided `jsonr` will exit with non 0 code.
...
If you may find `jsonr` useful for you navigate to [https://github.com/sobanieca/jsonr](https://github.com/sobanieca/jsonr) for installation instructions.

For sure this is not tool for everyone (especially for people that prefer working with GUI's). It may take some time to setup all http files and environments. 
But it's one time job that pays off in future when you need to repeat acting with some
API's.
