...
It doesn't aim to solve all possible problems out there. It's supposed to handle most of the common issues when manually interacting with API's.


* From time to time you need to test something on your sandbox environment. Instead of having to deal with clicking via UI, you can have a simple script that
  creates organization, creates some project:
...
1. The "Golden State" Environment Reset When developing or QA-ing, you often
   ...
   - The Workflow: A reset-env.http file containing a sequence of requests:
     ...
     - Why it's efficient: Instead of writing a complex bash script or manually
       ...
       requests in order. It’s a "push-button" solution that stays in your git
